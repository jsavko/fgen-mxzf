...
<script>
// Basic Config
let compendiumTypes = ['Actor', 'Card', 'Item', 'JournelEntry', 'Macro', 'Playlist', 'RollTable', 'Scene'];
let selected = 'Actor'
let siteAddr = 'https://fgen.mxzf.info/module/manifest/'

let packs = [
		{ name: '', label: '', type: 'Actor' },
	];
    ...
    function onSubmit(e) {
      ...
      for (let pair of packs) { 
          ...
          for (let data of Object.keys(pair)) {
              if ( data == 'name') {
                  pack_data['name'] = string_to_slug(pair.name);
              } else {
...
<main>
  <form on:submit|preventDefault={onSubmit}>
    ...
    <button on:click={clear} class="button secondary">
        Clear Packs
    </button>
